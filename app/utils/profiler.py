...
def profile_inference(classifier, image, iterations: int = 100) -> Dict[str, Any]:
    """
    ...
    if TORCH_PROFILER_AVAILABLE:
        ...
        try:
            ...
            results['torch_profiler'] = {
                'top_operations': sorted(torch_results, key=lambda x: x['cpu_time_total'], reverse=True)[:10]
            }
...
def benchmark_model_performance(classifier, test_images, iterations: int = 10) -> Dict[str, Any]:
    """
    ...
    for i, image in enumerate(test_images):
        logger.info(f"Benchmarking image {i+1}/{len(test_images)} (size: {image.size})")
...
def print_profiling_summary(results: Dict[str, Any]) -> None:
    ...
    if 'python_profiler' in results:
        ...
        print(f"Average per iteration: {py_prof['avg_time_per_iteration']*1000:.1f}ms")
        
        print("\nTop Functions by Cumulative Time:")
        for i, func in enumerate(py_prof['top_functions'][:5], 1):
            print(f"{i:2d}. {func['function']}")
            print(f"    Cumulative: {func['cumulative_time']:.3f}s, Total: {func['total_time']:.3f}s, Calls: {func['calls']}")
    
    # PyTorch profiler results
    if 'torch_profiler' in results and 'error' not in results['torch_profiler']:
        ...
        for i, op in enumerate(torch_prof['top_operations'][:5], 1):
            ...
            print(f"    CPU Time: {op['cpu_time_total']:.3f}ms, Count: {op['count']}")
