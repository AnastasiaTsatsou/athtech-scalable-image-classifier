...
def check_prerequisites():
    ...
    success, stdout, stderr = run_command("kubectl get crd inferenceservices.serving.kserve.io")
    if not success:
        print("❌ KServe operator not installed")
        print("Please install KServe operator first:")
        print("  kubectl apply -f https://github.com/kserve/kserve/releases/download/v0.11.2/kserve.yaml")
        return False
...
def deploy_model():
    ...
    for i in range(30):
        success, stdout, stderr = run_command("kubectl get pvc mobilenet-model-pvc -n kserve-comparison -o jsonpath='{.status.phase}'")
...
"""
    ...
    success, stdout, stderr = run_command("kubectl wait --for=condition=complete job/model-copy-job -n kserve-comparison --timeout=60s")
    if not success:
    ...
    success, stdout, stderr = run_command("kubectl apply -f inferenceservice.yaml")
    if not success:
...
def wait_for_ready():
    ...
    while time.time() - start_time < timeout:
        success, stdout, stderr = run_command("kubectl get inferenceservice mobilenet-v3-large -n kserve-comparison -o jsonpath='{.status.conditions[?(@.type==\"Ready\")].status}'")
        ...
        # Check for failure
        success, stdout, stderr = run_command("kubectl get inferenceservice mobilenet-v3-large -n kserve-comparison -o jsonpath='{.status.conditions[?(@.type==\"Ready\")].reason}'")
...
def setup_port_forward():
    ...
    # Get service name
    success, stdout, stderr = run_command("kubectl get svc -n kserve-comparison -l serving.kserve.io/inferenceservice=mobilenet-v3-large -o jsonpath='{.items[0].metadata.name}'")
    if not success:
    ...
    cmd = f"kubectl port-forward -n kserve-comparison svc/{service_name} 8081:80"
    print(f"Running: {cmd}")
    ...
    try:
        ...
        success, stdout, stderr = run_command("curl -s http://localhost:8081/ping", timeout=5)
        if success:
