...
def create_test_images() -> Dict[str, np.ndarray]:
    ...
    # Green image
    images["green"] = np.ones((224, 224, 3), dtype=np.float32) * [
        0.0,
        1.0,
        0.0,
    ]
        ...
        if response.status_code == 200:
            ...
            if "model_version_status" in metadata:
                ...
                for version in versions:
                    ...
                    print(
                        f"   Version {version.get('version', 'N/A')}: {state}"
                    )
...
def predict_image(
    url: str, model_name: str, image: np.ndarray, image_name: str = "test"
) -> Optional[Dict]:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(
                f"[ERROR] Prediction failed for {image_name}: {response.status_code}"
            )
            print(f"   Response: {response.text}")
            return None
...
def analyze_predictions(
    predictions: List[float], imagenet_classes: Dict[int, str], top_k: int = 5
...
def test_predictions(
        ...
        if result:
            ...
            if verbose:
                ...
                for i, (idx, class_name, confidence) in enumerate(top_results):
                    print(
                        f"      {i+1}. {class_name} ({idx}): {confidence:.4f}"
                    )
        else:
            if verbose:
    ...
    # Print summary
    print(
        f"[OK] Predictions completed: {success_count}/{len(test_images)} successful"
    )

    if all_results:
        # Calculate average response time
        avg_time = sum(r["response_time"] for r in all_results) / len(
            all_results
        )
...
def test_model_health(url: str) -> bool:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if "models" in models:
                model_names = [
                    m.get("name", "unknown") for m in models["models"]
                ]
...
def check_port_forward(namespace: str = "tfserving-simple") -> bool:
    ...
    try:
        ...
        if result.returncode == 0:
            print(
                f"[OK] Port-forward is running (PID: {result.stdout.strip()})"
            )
            return True
        else:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Test TensorFlow Serving MobileNetV3-Large deployment"
    )
    parser.add_argument(
        "--url", default="http://localhost:8082", help="TensorFlow Serving URL"
    )
    parser.add_argument(
        "--model", default="mobilenet_v3_large", help="Model name"
    )
    ...
    parser.add_argument(
        "--namespace", default="tfserving-simple", help="Kubernetes namespace"
    )
    ...
    if not test_predictions(
        args.url, args.model, test_images, imagenet_classes, args.verbose
    ):
