    ...
    for img_info in sample_images:
        try:
            ...
            if response.status_code == 200:
                ...
                downloaded_count += 1
                print(
                    f"✓ Downloaded {img_info['name']} ({img_info['description']})"
                )
            else:
                print(
                    f"⚠ Failed to download {img_info['name']}: {response.status_code}"
                )
...
def create_image_manifest():
    ...
    for img_file in images_dir.glob("*.jpg"):
        ...
        if img_file.name.startswith("synthetic_"):
            ...
            img_info["color"] = img_file.stem.split("_", 1)[1]
            manifest["synthetic_images"].append(img_info)
        elif img_file.name in [
            "checkerboard.jpg",
            "concentric_circles.jpg",
            "diagonal_stripes.jpg",
        ]:
    ...
    manifest["total_count"] = (
        len(manifest["synthetic_images"])
        + len(manifest["pattern_images"])
        + len(manifest["real_images"])
    )
