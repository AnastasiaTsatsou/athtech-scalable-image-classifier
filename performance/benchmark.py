...
class PerformanceBenchmark:
    ...
    def run_health_benchmark(
        self, duration: int = 60, rps: int = 10
    ) -> Dict[str, Any]:
        """Benchmark health endpoint"""
        logger.info(
            f"Running health endpoint benchmark: {duration}s at {rps} RPS"
        )
    ...
    def run_model_info_benchmark(
        self, duration: int = 60, rps: int = 10
    ) -> Dict[str, Any]:
        """Benchmark model info endpoint"""
        logger.info(
            f"Running model info endpoint benchmark: {duration}s at {rps} RPS"
        )
    ...
    def run_classification_benchmark(
        self, duration: int = 60, rps: int = 5, image_sizes: List[tuple] = None
    ) -> Dict[str, Any]:
        """Benchmark image classification endpoint"""
        logger.info(
            f"Running classification benchmark: {duration}s at {rps} RPS"
        )
        ...
        test_images = ImageGenerator.create_test_images(
            len(image_sizes), image_sizes
        )
    ...
    def run_stress_test(
        self, max_workers: int = 50, duration: int = 300
    ) -> Dict[str, Any]:
        """Run stress test with increasing load"""
        logger.info(
            f"Running stress test: {max_workers} workers for {duration}s"
        )
    ...
    def run_comprehensive_benchmark(self) -> Dict[str, Any]:
        ...
        benchmark_results["health"] = self.run_health_benchmark(
            duration=30, rps=20
        )

        # 2. Model info benchmark
        benchmark_results["model_info"] = self.run_model_info_benchmark(
            duration=30, rps=20
        )
        ...
        benchmark_results[
            "classification"
        ] = self.run_classification_benchmark(duration=60, rps=5)

        # 4. Latency benchmark
        benchmark_results["latency"] = self.run_latency_benchmark(
            iterations=100
        )
        ...
        # 6. Stress test
        benchmark_results["stress"] = self.run_stress_test(
            max_workers=20, duration=120
        )
    ...
    def _save_benchmark_results(
        self, test_name: str, results: List, analysis: Dict[str, Any]
    ):
        ...
        try:
            ...
            response = requests.get(
                f"{self.tester.base_url}/api/v1/model/info", timeout=10
            )
        ...
        results_file = os.path.join(
            self.results_dir, f"{test_name}_results_{timestamp}.json"
        )
        ...
        analysis_file = os.path.join(
            self.results_dir, f"{test_name}_analysis_{timestamp}.json"
        )
    ...
    def _generate_comprehensive_report(
        ...
        report_file = os.path.join(
            self.results_dir, f"comprehensive_report_{timestamp}.md"
        )
        ...
        with open(report_file, "w") as f:
            ...
            for test_name, results in benchmark_results.items():
                ...
                f.write(
                    f"- **Average Response Time**: {results['avg_response_time']:.3f}s\n"
                )
                f.write(
                    f"- **95th Percentile**: {results['p95_response_time']:.3f}s\n"
                )
                f.write(
                    f"- **99th Percentile**: {results['p99_response_time']:.3f}s\n"
                )
                f.write(
                    f"- **Requests per Second**: {results['requests_per_second']:.2f}\n\n"
                )
            ...
            f.write("## Summary\n\n")
            f.write(
                "This comprehensive benchmark provides insights into the performance "
            )
            f.write(
                "characteristics of the image classifier service under various load conditions.\n"
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Image Classifier Performance Benchmark"
    )
    parser.add_argument(
        "--url", default="http://localhost", help="Base URL of the service"
    )
    parser.add_argument(
        "--test",
        choices=[
    ...
    parser.add_argument(
        "--rps", type=int, default=10, help="Requests per second"
    )
    parser.add_argument(
        "--workers",
        type=int,
        default=50,
        help="Number of workers for stress test",
    )
    parser.add_argument(
        "--iterations",
        type=int,
        default=100,
        help="Number of iterations for latency/memory tests",
    )
