...
class LoadBalancingComparison:
    ...
    def test_load_balancing_performance(
        self, algorithm: str, iterations: int = 100
    ) -> Dict[str, Any]:
        """Test performance with specific load balancing algorithm"""
        logger.info(
            f"Testing {algorithm} load balancing with {iterations} iterations"
        )
        ...
        return {
            "algorithm": algorithm,
            "health": {
                "success_rate": health_success / 20
                if health_success > 0
                else 0,
                "stats": health_stats,
            },
            "classification": {
                "success_rate": classification_success / iterations
                if classification_success > 0
                else 0,
    ...
    def compare_algorithms(
        self, algorithms: List[str], iterations: int = 100
    ) -> Dict[str, Any]:
        ...
        for algorithm in algorithms:
            ...
            result = self.test_load_balancing_performance(
    ...
    def _generate_comparison_report(self, results: Dict[str, Any]):
        ...
        report_file = os.path.join(
            self.results_dir, f"load_balancing_comparison_{timestamp}.md"
        )
        ...
        with open(report_file, "w") as f:
            ...
            for algorithm, result in results.items():
                ...
                f.write(
                    f"| {algorithm.replace('_', ' ').title()} | {health_sr:.2%} | "
            ...
            for algorithm, result in results.items():
                ...
                if health["stats"]:
                    f.write(
                        f"- **Average Response Time**: {health['stats']['avg']:.3f}s\n"
                    )
                    f.write(
                        f"- **95th Percentile**: {health['stats']['p95']:.3f}s\n"
                    )
                f.write("\n")
                ...
                f.write(
                    f"- **Success Rate**: {classification['success_rate']:.2%}\n"
                )
                f.write(f"- **Iterations**: {classification['iterations']}\n")
                if classification["stats"]:
                    f.write(
                        f"- **Average Response Time**: {classification['stats']['avg']:.3f}s\n"
                    )
                    f.write(
                        f"- **Min Response Time**: {classification['stats']['min']:.3f}s\n"
                    )
                    f.write(
                        f"- **Max Response Time**: {classification['stats']['max']:.3f}s\n"
                    )
                    f.write(
                        f"- **95th Percentile**: {classification['stats']['p95']:.3f}s\n"
                    )
                    f.write(
                        f"- **99th Percentile**: {classification['stats']['p99']:.3f}s\n"
                    )
                f.write("\n")

            f.write("## Analysis\n\n")
            f.write(
                "This comparison provides insights into the performance characteristics "
            )
            f.write(
                "of different load balancing algorithms for ML workloads.\n\n"
            )

            f.write("### Key Findings\n\n")
            f.write(
                "1. **Algorithm Selection**: Different algorithms show varying performance "
            )
            f.write("characteristics under different load patterns.\n\n")

            f.write(
                "2. **Health Check Performance**: All algorithms should maintain high "
            )
            f.write("success rates for health checks.\n\n")

            f.write(
                "3. **Classification Performance**: The choice of algorithm can impact "
            )
        ...
        data_file = os.path.join(
            self.results_dir, f"load_balancing_data_{timestamp}.json"
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Load Balancing Algorithm Comparison"
    )
    parser.add_argument(
    ...
    for algorithm, result in results.items():
        ...
        print(
            f"  Classification Success Rate: {result['classification']['success_rate']:.2%}"
        )
        if result["classification"]["stats"]:
            print(
                f"  Avg Classification Time: {result['classification']['stats']['avg']:.3f}s"
            )
            print(
                f"  P95 Classification Time: {result['classification']['stats']['p95']:.3f}s"
            )
