...
class ImageGenerator:
    ...
    def create_test_images(
        count: int, sizes: List[tuple] = None
    ) -> List[bytes]:
...
class LoadTester:
    ...
    def test_model_info_endpoint(self) -> TestResult:
        ...
        try:
            response = requests.get(
                f"{self.base_url}/api/v1/model/info", timeout=self.timeout
            )
            response_time = time.time() - start_time
    ...
    def run_concurrent_tests(
        self, test_configs: List[Dict[str, Any]], max_workers: int = 10
    ) -> List[TestResult]:
    ...
    def run_load_test(
        ...
        **kwargs,
        ...
        logger.info(
            f"Starting load test: {test_type} for {duration_seconds}s at {requests_per_second} RPS"
        )
    ...
    def run_stress_test(
        ...
        **kwargs,
        ...
        logger.info(
            f"Starting stress test: {test_type} with {max_workers} workers for {duration_seconds}s"
        )
        ...
        for workers in range(1, max_workers + 1, 5):
            ...
            batch_results = self.run_concurrent_tests(
                test_configs, max_workers=workers
            )
            results.extend(batch_results)

            logger.info(
                f"Completed batch with {workers} workers. Results: {len(batch_results)}"
            )
    ...
    def analyze_results(self, results: List[TestResult]) -> Dict[str, Any]:
        ...
        analysis = {
            ...
            / (results[-1].timestamp - results[0].timestamp).total_seconds()
            if len(results) > 1
            else 0,
