...
class ImageClassifierUser(HttpUser):
    ...
    def test_model_info_endpoint(self):
        ...
        with self.client.get("/api/v1/model/info", catch_response=True) as response:
            if response.status_code == 200:
                response.success()
            else:
    ...
    def test_classify_endpoint(self):
        ...
        with self.client.post("/api/v1/classify", files=files, data=data, 
            ...
            if response.status_code == 200:
                ...
                response.failure(f"Classification failed with status {response.status_code}")
...
class ErrorTestingUser(HttpUser):
    ...
    def test_invalid_image(self):
        ...
        with self.client.post("/api/v1/classify", files=files, data=data, 
            ...
            if response.status_code == 400:
                ...
                response.failure(f"Expected 400 error, got {response.status_code}")
    ...
    def test_missing_image(self):
        ...
        with self.client.post("/api/v1/classify", data=data, 
                            catch_response=True) as response:
            if response.status_code == 400:
                response.success()
            else:
