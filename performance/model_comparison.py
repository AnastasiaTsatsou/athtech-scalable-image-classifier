...
class ModelComparison:
    ...
    def test_model_performance(
        self, model_name: str, iterations: int = 50
    ) -> Dict[str, Any]:
        ...
        try:
            response = requests.get(
                f"{self.base_url}/api/v1/model/info", timeout=10
            )
        ...
        for i in range(iterations):
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    logger.warning(
                        f"Classification failed for {model_name}: {response.status_code}"
                    )
    ...
    def compare_models(
        self, models: List[str], iterations: int = 50
    ) -> Dict[str, Any]:
    ...
    def _generate_comparison_report(self, results: Dict[str, Any]):
        ...
        report_file = os.path.join(
            self.results_dir, f"model_comparison_{timestamp}.md"
        )
        ...
        with open(report_file, "w") as f:
            ...
            for model_name, result in results.items():
                f.write(
                    f"| {model_name} | {result['success_rate']:.2%} | "
            ...
            for model_name, result in results.items():
                ...
                f.write(
                    f"- **Min Response Time**: {result['min_response_time']:.3f}s\n"
                )
                f.write(
                    f"- **Max Response Time**: {result['max_response_time']:.3f}s\n"
                )
                f.write(
                    f"- **95th Percentile**: {result['p95_response_time']:.3f}s\n"
                )
                f.write(
                    f"- **99th Percentile**: {result['p99_response_time']:.3f}s\n"
                )
                f.write(
                    f"- **Model Info Time**: {result['model_info_time']:.3f}s\n"
                )
                f.write(
                    f"- **Model Info Success**: {result['model_info_success']}\n\n"
                )
            ...
            f.write(
                "of different image classification models under identical test conditions.\n"
            )
        ...
        data_file = os.path.join(
            self.results_dir, f"model_comparison_data_{timestamp}.json"
        )
