...
class ComprehensivePerformanceBenchmark:
    ...
    def test_environment(self, base_url: str, environment_name: str, num_requests: int = 10) -> Dict[str, Any]:
        ...
        if response_times and processing_times:
            return {
                ...
                'processing_times': {
                    ...
                    'p95': sorted(processing_times)[int(0.95 * len(processing_times))],
                    'p99': sorted(processing_times)[int(0.99 * len(processing_times))]
                },
                'cache_performance': self._analyze_cache_performance(processing_times)
            }
        else:
            return {
                'environment': environment_name,
    ...
    def _analyze_cache_performance(self, processing_times: List[float]) -> Dict[str, Any]:
        """Analyze cache performance from processing times."""
        if len(processing_times) < 2:
            return {'cache_miss_time': 0, 'cache_hit_time': 0, 'cache_hit_rate': 0}
        ...
        return {
            ...
            'cache_hit_time': statistics.mean(cache_hit_times) if cache_hit_times else 0,
    ...
    def run_comprehensive_benchmark(self) -> Dict[str, Any]:
        ...
        # Generate summary
        benchmark_results['summary'] = self._generate_summary(benchmark_results['environments'])
        
        return benchmark_results
    
    def _generate_summary(self, environments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for env_name, result in environments.items():
            if 'processing_times' in result:
                ...
                summary['cache_performance'][env_name] = result['cache_performance']
        
        # Generate recommendations
        if summary['target_achieved']:
            achieved_envs = [env for env, achieved in summary['target_achieved'].items() if achieved]
            if achieved_envs:
                summary['recommendations'].append(f"✅ Target achieved in: {', '.join(achieved_envs)}")
            
            failed_envs = [env for env, achieved in summary['target_achieved'].items() if not achieved]
            if failed_envs:
                summary['recommendations'].append(f"❌ Target not achieved in: {', '.join(failed_envs)}")
    ...
    def print_results(self, results: Dict[str, Any]):
        ...
        for env_name, result in results['environments'].items():
            ...
            if 'processing_times' in result:
                print(f"✅ Successful Requests: {result['successful_requests']}/{result['num_requests']}")
                print(f"📊 Processing Time P95: {result['processing_times']['p95']:.1f}ms")
                print(f"📊 Processing Time Avg: {result['processing_times']['avg']:.1f}ms")
                print(f"📊 Response Time P95: {result['response_times']['p95']:.1f}ms")
                print(f"📊 Response Time Avg: {result['response_times']['avg']:.1f}ms")
                ...
                print(f"🎯 Target (<200ms): {'✅ ACHIEVED' if target_met else '❌ NOT MET'}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Run comprehensive performance benchmark')
