...
class PerformanceTestHelper:
    ...
    @staticmethod
    def create_test_image(size: tuple = (224, 224), color: str = 'red') -> Image.Image:
    ...
    @staticmethod
    def test_endpoint_availability(base_url: str, endpoint: str = '/api/v1/health') -> bool:
    ...
    def format_performance_summary(result: PerformanceResult) -> str:
        ...
        response_stats = PerformanceTestHelper.calculate_percentiles(result.response_times)
        processing_stats = PerformanceTestHelper.calculate_percentiles(result.processing_times)
...
class LoadTestRunner:
    ...
    def run_burst_test(self, burst_size: int = 5, delay: float = 0.1) -> List[Dict[str, Any]]:
    ...
    def run_sustained_test(self, duration_seconds: int = 30, interval: float = 1.0) -> List[Dict[str, Any]]:
    ...
    def run_cache_test(self, num_requests: int = 10) -> Dict[str, Any]:
        ...
        if results and results[0]['success']:
            ...
            cache_hit_times = [r['processing_time_ms'] for r in results[1:] if r['success']]
...
def run_quick_performance_check(base_url: str = "http://localhost:8000") -> Dict[str, Any]:
    ...
    if successful_results:
        ...
        response_stats = PerformanceTestHelper.calculate_percentiles(response_times)
        processing_stats = PerformanceTestHelper.calculate_percentiles(processing_times)
...
if __name__ == "__main__":
    ...
    if result['available']:
        ...
        if 'processing_stats' in result:
            print(f"📊 P95 Processing Time: {result['processing_stats']['p95']:.1f}ms")
            print(f"🎯 Target Achievement: {'✅ ACHIEVED' if result['target_achieved'] else '❌ NOT MET'}")
        else:
            print(f"❌ Error: {result.get('error', 'Unknown error')}")
    else:
        print(f"❌ Service not available: {result.get('error', 'Unknown error')}")
