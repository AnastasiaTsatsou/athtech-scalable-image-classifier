...
class DockerComposePerformanceTester:
    ...
    def test_api_through_load_balancer(
    ...
    def test_batch_through_load_balancer(
...
def test_docker_compose_performance():
    ...
    print(
        f"Nginx Load Balancer Health: {health_result['response_time_ms']:.1f}ms"
    )
    assert (
    ...
    if results["response_times"] and results["processing_times"]:
        ...
        assert (
            p95_processing < 1000
        ), f"Docker Compose processing time {p95_processing:.1f}ms too slow"
