...
class KubernetesPerformanceTester:
    ...
    def test_kubernetes_cache_performance(
        self, num_requests: int = 10
    ) -> Dict[str, float]:
...
def test_kubernetes_performance():
    ...
    print(
        f"Kubernetes Health Check: {health_result['response_time_ms']:.1f}ms"
    )
    ...
    if results["response_times"] and results["processing_times"]:
        ...
        assert (
            p95_processing < 200
        ), f"Kubernetes processing time {p95_processing:.1f}ms exceeds 200ms target"
    ...
    print(
        f"Cache Hit Rate: {cache_results['cache_hit_count']}/{cache_results['total_requests']}"
    )

    # Cache hits should be very fast
    assert (
        cache_results["avg_cache_hit_time_ms"] < 10
    ), f"Kubernetes cache hit time {cache_results['avg_cache_hit_time_ms']:.1f}ms too slow"
